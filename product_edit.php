...
if (isset($_GET['pid']) && !empty($_GET['pid'])) { //check if a query has been sent
  ...
  try {
      $statement = $dbh->prepare('
        SELECT 
            product.productName, product.productDesc, product.productTags, image.imageReference
        FROM 
            product, image, company, user
        WHERE 
            company.companyID = user.company_companyID AND user.userID = product.user_userID AND product.productID = image.product_productID AND product.productID=:pid
      ');
...
$cid = $_SESSION['cid']; //Store CID so I can use it to query other results from the same company
  ...
  try {
      $statement = $dbh->prepare('
        ...
        FROM 
            product, image, company, user
        WHERE 
              company.companyID = user.company_companyID AND user.userID = product.user_userID AND product.productID = image.product_productIDAND company.companyID=:cid AND product.productID <> :pid ORDER BY product.productID DESC LIMIT 2
      ');
  ...
  <body>
    ...
    <div class="page-content">
      ...
      <section class="cart-message">
        ...
        <a class="btn-outlined-invert btn-success btn-sm" href="shop-single-item-v2.html">View cart</a>
      ...
      <section class="catalog-single">
          ...
          <div class="row">
            ...
            <div class="col-lg-6 col-md-6">
                    <form action="process_prod_update.php" method="POST" enctype="multipart/form-data" autocomplete="on">
                    <input type="hidden" name="pid" value="<?PHP echo $_GET['pid']; ?>" />
                    <div class="form-group">
                        <label for="ot_id">Product Name</label>                        
                        <input class="form-control" type="text" name="productName" id="ot_id" placeholder="Product Name" value="<?PHP echo $results[0]; ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="ot_id">Product Description</label>
                        <textarea class="form-control" rows="10" id="cs-message" placeholder="Product Description" name="productDesc" required><?PHP echo $results[1]; ?></textarea>            
                    </div>
                    <div class="form-group">
                        <label for="tags">Tags</label>
                        <input class="form-control" type="text" name="productTags" id="singleFieldTags2" placeholder="Product Tags" value="<?PHP echo $results[2]; ?>" required>
                    </div>
                    <div class="form-group">
                        <label for="ot_email">Image *disabled*</label>
                        <input class="form-control" type="file" name="productImage" id="ot_email" disabled="true">
                    </div>
            ...
            <div class="col-lg-6 col-md-6">
            	<div class="prod-gal master-slider" id="prod-gal">
              	<!--Slide1-->
                <div class="ms-slide">
                	<img src="uploads/large/<?PHP echo $imgL; ?>" data-src="uploads/large/<?PHP echo $imgL; ?>" alt="Lorem ipsum"/>
                  <img class="ms-thumb" src="uploads/large/<?PHP echo $imgL; ?>" alt="thumb" />
                </div>
                </div>
              </div>
            </div>
